...
function search_items(string $q, int $limit = 10, int $offset = 0): array
{
    ...
    $stmt = db()->prepare('SELECT * FROM items WHERE title LIKE :q AND TRIM(COALESCE(content_id, "")) <> "" AND TRIM(COALESCE(affiliate_url, "")) <> "" AND (affiliate_url LIKE :dmm1 OR affiliate_url LIKE :dmm2) ORDER BY release_date DESC, id DESC LIMIT :limit OFFSET :offset');
