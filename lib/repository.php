...
function search_items(string $q, int $limit = 10, int $offset = 0): array
{
    ...
    $stmt = db()->prepare('SELECT * FROM items WHERE title LIKE :q AND TRIM(COALESCE(content_id, "")) <> "" AND TRIM(COALESCE(service_code, "")) <> "" AND TRIM(COALESCE(floor_code, "")) <> "" ORDER BY release_date DESC, id DESC LIMIT :limit OFFSET :offset');
