...
function fetch_related_series_by_actress(int $actressId, int $limit = 50): array
{
    ...
    $stmt = db()->prepare(
        'SELECT DISTINCT sm.*
         ...
         INNER JOIN item_series isr ON ((isr.dmm_id IS NOT NULL AND isr.dmm_id != \'\' AND isr.dmm_id = sm.dmm_id) OR isr.series_name = sm.name)
         INNER JOIN item_actresses ia ON ia.item_id = isr.item_id
         INNER JOIN actresses a ON ((ia.dmm_id IS NOT NULL AND ia.dmm_id != \'\' AND a.dmm_id = ia.dmm_id) OR ia.actress_name = a.name)
         WHERE a.id = :id
...
function fetch_related_makers_by_actress(int $actressId, int $limit = 50): array
{
    ...
    $stmt = db()->prepare(
        'SELECT DISTINCT m.*
         FROM makers m
         INNER JOIN item_makers im ON ((im.dmm_id IS NOT NULL AND im.dmm_id != \'\' AND im.dmm_id = m.dmm_id) OR im.maker_name = m.name)
...
function fetch_items_by_series(int $seriesId, int $limit, int $offset = 0): array
{
    ...
    $stmt = db()->prepare(
        'SELECT i.*
         ...
         INNER JOIN series_master sm ON ((isr.dmm_id IS NOT NULL AND isr.dmm_id != \'\' AND sm.dmm_id = isr.dmm_id) OR isr.series_name = sm.name)
         WHERE sm.id = :id
         ORDER BY i.release_date DESC, i.id DESC
         LIMIT :limit OFFSET :offset'
    );
...
function fetch_item_actresses(string $contentId): array
{
    ...
    $stmt = db()->prepare(
        'SELECT DISTINCT a.*
         FROM actresses a
         INNER JOIN item_actresses ia ON ((ia.dmm_id IS NOT NULL AND ia.dmm_id != \'\' AND ia.dmm_id = a.dmm_id) OR ia.actress_name = a.name)
...
function fetch_item_genres(string $contentId): array
{
    ...
    $stmt = db()->prepare(
        'SELECT DISTINCT g.*
         FROM genres g
         INNER JOIN item_genres ig ON ((ig.dmm_id IS NOT NULL AND ig.dmm_id != \'\' AND ig.dmm_id = g.dmm_id) OR ig.genre_name = g.name)
