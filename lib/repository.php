...
function search_items(string $q, int $limit = 10, int $offset = 0): array
{
    ...
    $stmt = db()->prepare(
        'SELECT i.*
         ...
         AND (i.url LIKE :dmm3 OR i.url LIKE :dmm4)
         AND (i.image_small LIKE :http1 OR i.image_large LIKE :http2)
    ...
    $stmt->bindValue(':dmm3', '%dmm.co.jp%', PDO::PARAM_STR);
    $stmt->bindValue(':dmm4', '%fanza%', PDO::PARAM_STR);
    $stmt->bindValue(':http1', 'http%', PDO::PARAM_STR);
    $stmt->bindValue(':http2', 'http%', PDO::PARAM_STR);
    $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
    $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
    $stmt->execute();
