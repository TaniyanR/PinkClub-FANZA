...
### 商品・分類ページとセットアップの復旧（2026-09）

商品詳細と共通表示部品は、FANZA自身の `fc753287`（2026-09-15）の実装へ復旧しています。FANZAのAPI、商品ID、既存URL、画像・動画の扱いを維持します。漢字・数字等で始まるシリーズも一覧の「その他」に表示します。

1. 更新前にDBと `config.local.php` をバックアップし、修正版のコードを配置してください。既存の `config.local.php` は上書きしません。
2. 未セットアップ環境は `/public/setup_check.php` でサーバーパネルのDB情報を保存し、「セットアップを実行する」を押します。DB・テーブルの削除は不要です。
3. 接続成功時だけ設定を保存します。初回はパスワード入力が必要で、保存済みの場合は空欄で既存値を維持します。
4. MySQL `1045` は認証情報・接続元ホスト、`1044` は対象DBへの権限、`1049` はDB名を確認してください。実サーバーの認証情報や権限はコード更新だけでは変更されません。
5. インストール済み環境の未適用更新は、従来どおり `php scripts/init_db.php` で適用できます。管理者・商品・設定を保持します。DB障害を理由にセットアップ保護を解除する必要はありません。

新規セットアップ時は固定ページ用テーブルを先に作成し、テーブル作成前の判定キャッシュを破棄します。既存のマイグレーション名と適用履歴は維持します。公開HTMLキャッシュは今回の復旧用キーへ切り替わるため手動削除は不要で、TTLは変更しません。

#### 復旧の回帰テスト

PHP CLI（PDO MySQL / mysqli / mbstring / XML拡張）、Python 3、MySQLまたはMariaDBを使用します。専用の**空の** `pcf_test_*` DBと、そのDBにテーブル作成・更新権限を持つユーザーを用意し、次の環境変数を設定して実行してください。

```sh
export PCF_TEST_DB_HOST=127.0.0.1
export PCF_TEST_DB_PORT=3306
export PCF_TEST_DB_NAME=pcf_test_repair
export PCF_TEST_DB_USER=your_test_user
read -r -s -p 'Test DB password: ' PCF_TEST_DB_PASS
export PCF_TEST_DB_PASS
python3 tests/repair_smoke.py
```

アプリは一時コピー上で動かし、元の `config.local.php` を読み書きしません。テストDBに既存テーブルがあれば中止し、DBの削除・リセットは行いません。終了後はテストデータが残ります。PHPの実行ファイルを指定する場合は `PCF_PHP_BIN` を設定してください。検証内容はDB設定保存、CSRF、初期セットアップ、ログイン、再実行時のデータ保持、公開15ページ、管理3ページ、分類の遅延読み込み、404、URL転送、OGP、キャッシュ、RSS、Sitemapです。PHP内蔵サーバーのため、実サーバーのApache設定・外部API・外部画像配信は別途確認します。
