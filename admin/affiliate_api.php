...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $masterActionMap = [
        'test_actress_api' => 'actress',
        'test_genre_api' => 'genre',
        'test_maker_api' => 'maker',
        'test_series_api' => 'series',
        'test_author_api' => 'author',
    ];

    if (isset($masterActionMap[$action])) {
        try {
            $cfg = settings_get();
            if ((string)($cfg['api_id'] ?? '') === '' || (string)($cfg['affiliate_id'] ?? '') === '') {
                throw new RuntimeException('API ID / アフィリエイトID を保存してから実行してください。');
            ...
            $kind = $masterActionMap[$action];
            $masterTestResults = [run_master_test_fetch($kind, $cfg)];

            $resultType = 'success';
            $resultMessage = (string)($masterTestResults[0]['label'] ?? 'マスタAPI') . ' のテスト取得が完了しました（10件）。';
        } catch (Throwable $e) {
            $resultType = 'error';
...
<section class="admin-card admin-card--form">
  ...
  <form method="post">
    ...
    <div class="admin-actions">
      ...
      <button class="button-secondary" type="submit" name="action" value="test_actress_api">女優検索APIを10件テスト取得</button>
      <button class="button-secondary" type="submit" name="action" value="test_genre_api">ジャンル検索APIを10件テスト取得</button>
      <button class="button-secondary" type="submit" name="action" value="test_maker_api">メーカー検索APIを10件テスト取得</button>
      <button class="button-secondary" type="submit" name="action" value="test_series_api">シリーズ検索APIを10件テスト取得</button>
      <button class="button-secondary" type="submit" name="action" value="test_author_api">作者検索APIを10件テスト取得</button>
  ...
  <?php if ($masterTestResults !== []): ?>
    <div class="admin-card" style="margin-top:12px;">
      <h2 style="margin-top:0;">マスタAPI テスト取得結果（10件）</h2>
