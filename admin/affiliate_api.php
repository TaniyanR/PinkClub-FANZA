...
function run_item_test_sync(array $cfg, int $offset): array
{
    $syncService = dmm_sync_service();
    $method = new ReflectionMethod($syncService, 'syncItemsBatch');
    $paramCount = $method->getNumberOfParameters();

    if ($paramCount >= 5) {
        return $syncService->syncItemsBatch(
            (string)($cfg['site'] ?? 'FANZA'),
            (string)($cfg['service'] ?? 'digital'),
            (string)($cfg['floor'] ?? 'videoa'),
            10,
            $offset
        );
    }
    ...
    if ($action === 'test_items' || isset($masterActionMap[$action])) {
        try {
            ...
            if ($action === 'test_items') {
                ...
                $result = run_item_test_sync($cfg, $offset);
