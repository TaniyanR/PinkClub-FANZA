    ...
    try {
        ...
        if (is_array($discovered) && $discovered !== []) {
            ...
            foreach ($discovered as $page) {
                ...
                $file = basename((string)($page['file'] ?? $page['path'] ?? ''));
...
$titleText = (string)($title ?? (defined('APP_NAME') ? APP_NAME : 'Admin'));
?>
<!doctype html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
  <title><?= function_exists('e') ? e($titleText) : htmlspecialchars($titleText, ENT_QUOTES, 'UTF-8') ?></title>
...
<header class="admin-topbar">
  <div class="admin-topbar__brand">
    <a href="<?= function_exists('e') && function_exists('admin_url') ? e(admin_url('index.php')) : '/admin/index.php' ?>">PinkClub FANZA 管理</a>
  </div>
  ...
  <aside class="admin-sidebar" aria-label="管理メニュー">
    <nav>
      ...
      <?php if (!$menuAutoGenerated): ?>
        <p class="admin-form-note">固定メニュー</p>
      <?php endif; ?>
      <ul class="admin-sidebar__list">
        <?php foreach ($menuItems as $item): ?>
          <?php
            ...
            $itemScript = basename((string)parse_url($file, PHP_URL_PATH));
            $isActive = ($currentScript === $itemScript);
            $href = (function_exists('admin_url')) ? admin_url($file) : ('/admin/' . ltrim($file, '/'));
          ?>
          <li>
            <a class="admin-menu__link <?= $isActive ? 'is-active' : '' ?>"
               href="<?= function_exists('e') ? e($href) : htmlspecialchars($href, ENT_QUOTES, 'UTF-8') ?>">
              <?= function_exists('e') ? e($label) : htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?>
            </a>
          </li>
        <?php endforeach; ?>
      </ul>
