...
if (!function_exists('pcf_render_item_card')) {
    function pcf_render_item_card(array $item): void
    {
        ...
        echo '<article class="pcf-card pcf-item-card">';
        echo '<a class="pcf-item-card__thumb-link" href="' . e(public_url('item.php?id=' . (int)($item['id'] ?? 0))) . '">';
        echo '<img class="pcf-item-card__thumb" src="' . e(pcf_item_image($item)) . '" alt="' . e($title) . '" loading="lazy">';
        echo '</a>';
        echo '<div class="pcf-item-card__body">';
        echo '<h3 class="pcf-item-card__title"><a href="' . e(public_url('item.php?id=' . (int)($item['id'] ?? 0))) . '">' . e($title) . '</a></h3>';
        ...
        echo '</ul>';
        echo '<a class="pcf-btn" href="' . e(public_url('item.php?id=' . (int)($item['id'] ?? 0))) . '">詳細を見る</a>';
        echo '</div>';
        echo '</article>';
    }
}
