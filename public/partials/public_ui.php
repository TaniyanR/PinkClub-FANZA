...
if (!function_exists('pcf_render_actress_card')) {
    function pcf_render_actress_card(array $actress): void
    {
        $id = (int)($actress['id'] ?? 0);
        ...
        $url = public_url('actress.php') . '?id=' . rawurlencode((string)$id);
        $image = pcf_first_image_by_keys($actress, ['image_url', 'thumb_url', 'photo_url']);
        if ($image === '') {
            $image = pcf_placeholder_data_uri($name);
        }

        echo '<article class="pcf-actress-card">';
        echo '<a href="' . e($url) . '" class="pcf-actress-card__link">';
        echo '<div class="pcf-actress-card__thumb-wrap">';
        echo '<img class="pcf-actress-card__thumb" src="' . e($image) . '" alt="' . e($name) . '" loading="lazy" decoding="async">';
        echo '</div>';
        echo '<h3 class="pcf-actress-card__name">' . e($name) . '</h3>';
        if (!empty($actress['item_count']) && (int)$actress['item_count'] > 0) {
            echo '<span class="pcf-actress-card__count">' . number_format((int)$actress['item_count']) . '作品</span>';
        }
        echo '</a>';
        echo '</article>';
    }
...
if (!function_exists('pcf_render_actress_grid')) {
    function pcf_render_actress_grid(array $actresses): void
    {
        if ($actresses === []) {
            return;
        }

        echo '<div class="pcf-actress-grid">';
        foreach ($actresses as $actress) {
            if (is_array($actress)) {
                pcf_render_actress_card($actress);
            }
        }
        echo '</div>';
    }
...
if (!function_exists('pcf_render_genre_chip')) {
    function pcf_render_genre_chip(array $genre): void
    {
        $id = (int)($genre['id'] ?? 0);
        ...
        $url = public_url('genre.php') . '?id=' . rawurlencode((string)$id);
        echo '<a href="' . e($url) . '" class="pcf-chip">';
        echo '<span class="pcf-chip__name">' . e($name) . '</span>';
        if (!empty($genre['item_count']) && (int)$genre['item_count'] > 0) {
            echo '<span class="pcf-chip__count">' . number_format((int)$genre['item_count']) . '</span>';
        }
        echo '</a>';
    }
}

if (!function_exists('pcf_render_genre_list')) {
    function pcf_render_genre_list(array $genres): void
    {
        if ($genres === []) {
            return;
        }

        echo '<div class="pcf-chip-list">';
        foreach ($genres as $genre) {
            if (is_array($genre)) {
                pcf_render_genre_chip($genre);
            }
        }
        echo '</div>';
    }
...
if (!function_exists('pcf_render_maker_chip')) {
    function pcf_render_maker_chip(array $maker): void
    {
        $id = (int)($maker['id'] ?? 0);
        ...
        $url = public_url('maker.php') . '?id=' . rawurlencode((string)$id);
        echo '<a href="' . e($url) . '" class="pcf-chip pcf-chip--maker">';
        echo '<span class="pcf-chip__name">' . e($name) . '</span>';
        if (!empty($maker['item_count']) && (int)$maker['item_count'] > 0) {
            echo '<span class="pcf-chip__count">' . number_format((int)$maker['item_count']) . '</span>';
        }
        echo '</a>';
    }
}

if (!function_exists('pcf_render_maker_list')) {
    function pcf_render_maker_list(array $makers): void
    {
        if ($makers === []) {
            return;
        }
...
if (!function_exists('pcf_render_series_chip')) {
    function pcf_render_series_chip(array $series): void
    {
        $id = (int)($series['id'] ?? 0);
        ...
        $url = public_url('series_detail.php') . '?id=' . rawurlencode((string)$id);
        echo '<a href="' . e($url) . '" class="pcf-chip pcf-chip--series">';
        echo '<span class="pcf-chip__name">' . e($name) . '</span>';
        if (!empty($series['item_count']) && (int)$series['item_count'] > 0) {
            echo '<span class="pcf-chip__count">' . number_format((int)$series['item_count']) . '</span>';
        }
        echo '</a>';
    }
}

if (!function_exists('pcf_render_series_list')) {
    function pcf_render_series_list(array $seriesList): void
    {
        if ($seriesList === []) {
            return;
        }
...
if (!function_exists('pcf_render_banner')) {
    function pcf_render_banner(string $position = 'top'): void
    {
        // 既存のバナー表示ロジック用スタブ
    }
}
