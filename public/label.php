...
function pcf_fetch_label_access_ranking(string $periodFrom): array
{
    try {
        ...
        $stmt = db()->prepare("SELECT il.label_name AS name, COUNT(ol.id) AS access_count FROM out_logs ol INNER JOIN items i ON i.affiliate_url = ol.target_url INNER JOIN item_labels il ON i.content_id = il.content_id WHERE ol.created_at >= :period_from AND TRIM(COALESCE(i.affiliate_url, '')) <> '' AND TRIM(COALESCE(il.label_name, '')) <> '' GROUP BY il.label_name ORDER BY access_count DESC, il.label_name ASC LIMIT 200");
...
$labelName = trim((string)get('name', ''));
...
if ($labelName !== '') {
    try {
        $rows = dedupe_items_by_key(fetch_items_by_label_name($labelName, $per + 1, $offset));
        [$list, $hasNext] = paginate_items($rows, $per);
    } catch (Throwable) {
        $list = [];
        $hasNext = false;
    }
...
$canonicalUrl = public_url('label.php') . '?' . http_build_query(['name' => $labelName]);
if ($page > 1) {
    $relPrev = public_url('label.php') . '?' . http_build_query(['name' => $labelName, 'page' => $page - 1]);
}
if ($hasNext) {
    $relNext = public_url('label.php') . '?' . http_build_query(['name' => $labelName, 'page' => $page + 1]);
}
...
<?php if ($list !== []): ?>
  ...
  <nav class="pcf-pagination" aria-label="ページネーション">
    <?php if ($page > 1): ?>
      <a class="pcf-pagination__link" href="<?= e(public_url('label.php') . '?' . http_build_query(['name' => $labelName, 'page' => $page - 1])) ?>">前へ</a>
    <?php endif; ?>
    <span class="pcf-pagination__link is-current"><?= e((string)$page) ?></span>
    <?php if ($hasNext): ?>
      <a class="pcf-pagination__link" href="<?= e(public_url('label.php') . '?' . http_build_query(['name' => $labelName, 'page' => $page + 1])) ?>">次へ</a>
    <?php endif; ?>
  </nav>
<?php else: ?>
...
<section id="access-ranking" class="block" style="margin-top:24px;">
  ...
  <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:8px;">
    <?php foreach ($accessRankingTabs as $tabKey => $tabConfig): ?>
      <?php $tabUrl = public_url('label.php') . '?' . http_build_query(['name' => $labelName, 'rank_period' => (string)$tabKey]) . '#access-ranking'; ?>
      ...
      <table style="width:100%; border-collapse:collapse; table-layout:fixed;">
        ...
        <tbody>
          <?php foreach ($accessRankingRows as $accessIndex => $accessRow): ?>
            <?php $labelUrl = public_url('label.php') . '?' . http_build_query(['name' => (string)($accessRow['name'] ?? '')]); ?>
            <tr>
