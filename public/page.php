...
if (db_table_exists('fixed_pages')) {
    ...
    if (is_array($p) && $slug === 'privacy-policy' && strpos((string)($p['body'] ?? ''), $privacyPolicyContactSection) !== false) {
        $oldBody = (string)$p['body'];
        $p['body'] = str_replace($privacyPolicyContactSection, '', $oldBody);
        db()->prepare('UPDATE fixed_pages SET body=:body, updated_at=NOW() WHERE slug="privacy-policy" AND body=:old_body')->execute([
            ':body' => $p['body'],
            ':old_body' => $oldBody,
        ]);
...
if ($slug === 'privacy-policy') {
    $p['body'] = str_replace($privacyPolicyContactSection, '', (string)$p['body']);
    $p['body'] = str_replace(
        ['[サイト名]', '[サイトURL]', '[サイトRSS]'],
