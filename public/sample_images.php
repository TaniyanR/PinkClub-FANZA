...
<head>
  ...
  <style>
    html { height: 100%; }
    body { height: calc(100% - 24px); font-family: Arial, sans-serif; margin: 12px; background: #f8f9fa; overflow: hidden; display: flex; flex-direction: column; }
    h1 { flex: 0 0 auto; font-size: 18px; margin-bottom: 12px; position: sticky; top: 0; background: #f8f9fa; padding: 6px 0; }
    .message { text-align: center; color: #555; margin-top: 32px; }
    .sample-scroll { flex: 1 1 auto; min-height: 0; display: flex; gap: 10px; overflow-x: auto; overflow-y: hidden; padding-bottom: 6px; align-items: stretch; }
    .sample-scroll::-webkit-scrollbar { height: 10px; }
    .sample-scroll::-webkit-scrollbar-thumb { background: #b9bdc5; border-radius: 8px; }
    .sample-frame { width: min(720px, 88vw); flex: 0 0 min(720px, 88vw); max-width: none; background: #fff; border: 1px solid #dcdcde; margin: 0; display: flex; align-items: center; justify-content: center; box-sizing: border-box; }
    .sample-frame img { width: 100%; height: 100%; object-fit: contain; display: block; }
  </style>
</head>
<body>
