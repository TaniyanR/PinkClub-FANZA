...
try {
    ...
    $dbError = true;
    error_log('public/item.php db connect failed: ' . $e->getMessage());
    $pdo = null;
}
...
if ($pdo instanceof PDO) {
    if ($id > 0) {
        ...
        $stmt = $pdo->prepare('SELECT * FROM items WHERE content_id = ?');
    ...
    try {
        ...
        error_log('public/item.php view_count update failed: ' . $e->getMessage());
    }

    try {
        $relatedSections = item_fetch_related_sections($pdo, $item);
    } catch (Throwable $e) {
        error_log('public/item.php related sections failed: ' . $e->getMessage());
        $relatedSections = [];
    }
}
