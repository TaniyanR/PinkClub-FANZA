...
$sampleThumbSlots = [];
$sampleThumbMax = 22;
for ($i = 0; $i < $sampleThumbMax; $i++) {
    $sampleThumbSlots[] = $sampleImagesSmallLargeMap[$i] ?? null;
}
...
<article>
  ...
  <?php if ($sampleMovieUrl !== '' || $sampleImagesSmallLargeMap !== []): ?>
    <div style="display:flex; gap:12px; align-items:flex-start; flex-wrap:wrap;">
      ...
      <?php endif; ?>
      <?php if ($sampleThumbSlots !== []): ?>
      <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:4px; width:120px; height:480px;">
        <?php foreach ($sampleThumbSlots as $i => $imagePair): ?>
          <?php if (is_array($imagePair)): ?>
          <a href="<?= e((string)$imagePair['large']) ?>" class="pcf-image-viewer-trigger" data-image-index="<?= e((string)$i) ?>" style="display:block;">
            <img src="<?= e((string)$imagePair['small']) ?>" alt="サンプル画像 <?= e((string)($i + 1)) ?>" loading="lazy" style="display:block; width:100%; height:39px; object-fit:cover;">
          </a>
          <?php else: ?>
          <span style="display:block; width:100%; height:39px;"></span>
          <?php endif; ?>
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
