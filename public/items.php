...
try {
    $total = (int)db()->query("SELECT COUNT(*) FROM items WHERE TRIM(COALESCE(title, '')) <> ''")->fetchColumn();
} catch (Throwable) {
    $total = 0;
}
...
foreach ($orderSqlCandidates as $orderSql) {
    try {
        $stmt = db()->prepare('SELECT * FROM items WHERE TRIM(COALESCE(title, "")) <> "" ORDER BY ' . $orderSql . ' LIMIT :l OFFSET :o');
