...
function fetch_items_with_order_fallback(PDO $pdo, array $orderByCandidates, int $limit, int $offset = 0): array
{
    ...
    foreach ($orderByCandidates as $orderBy) {
        $rows = query_all_safe($pdo, 'SELECT * FROM items' . $sourceWhereSql . ' ORDER BY ' . $orderBy . ' LIMIT :limit OFFSET :offset', [':limit' => $limit, ':offset' => $offset]);
        if ($rows !== []) {
            return $rows;
        }
    }
...
try {
    ...
    if ($itemCount > 0) {
        $pg = paginate($itemCount, $page, $limit);
        $page = (int)$pg['page'];
        $pages = (int)$pg['pages'];
        $offset = (int)$pg['offset'];
        $latestItems = fetch_items_with_order_fallback($pdo, [
            'release_date DESC, updated_at DESC, id DESC',
            'date_published DESC, updated_at DESC, id DESC',
            'updated_at DESC, id DESC',
            'id DESC',
        ], $limit, $offset);
        $displayItems = array_values(array_filter($latestItems, 'is_array'));
        $hasNextPage = $page < $pages;
        $fallbackItems = array_slice($displayItems, 0, 12);
    }
} catch (Throwable $e) {
