...
function collect_newest_items_for_listing(string $whereSql): array
{
    ...
    while (true) {
        $stmt = db()->prepare('SELECT * FROM items' . $whereSql . ' ORDER BY release_date DESC, id DESC LIMIT :l OFFSET :o');
        ...
        $displayable = dedupe_items_for_listing(array_merge($displayable, array_values(array_filter($chunk, static fn(array $row): bool => is_displayable_item_for_listing($row)))));
        $cursor += count($chunk);
