  ...
  <section class="item-card-list">
    <?php foreach ($rows as $r): ?>
      ...
      if ($titleText === '') {
          ...
          $titleText = trim((string)($raw['title'] ?? $raw['iteminfo']['title'] ?? ''));
      }
      if ($titleText === '') {
          $titleText = 'おじさんとの中出しセックスBEST480分';
      }
      $imageUrl = '';
      $fullPackageUrl = trim((string)($item['full_package_url'] ?? ''));
      $packageImageLarge = trim((string)($item['package_image_large'] ?? ''));
      $mainImageUrl = trim((string)($item['main_image_url'] ?? $item['image_url'] ?? ''));
      if ($fullPackageUrl !== '') {
          $imageUrl = items_normalize_image_url($fullPackageUrl);
      }
      if ($imageUrl === '' && $packageImageLarge !== '') {
          $imageUrl = items_normalize_image_url($packageImageLarge);
      }
      if ($imageUrl === '' && $mainImageUrl !== '') {
          $imageUrl = items_normalize_image_url($mainImageUrl);
      }
      if ($imageUrl === '') {
          $imageUrl = items_normalize_image_url(items_pick_full_package_image($item));
      }
      ...
      if ($sampleMovieUrl === '') {
          $sampleMovieUrl = $itemUrl;
      }
      $sampleImagesUrl = $contentId !== '' ? public_url('sample_images.php?content_id=' . rawurlencode($contentId)) : $itemUrl;
      ?>
      <div class="item-card">
        <a href="<?= e($itemUrl) ?>" class="item-card__image-link">
          <?php if ($imageUrl !== ''): ?>
            <img src="<?= e($imageUrl) ?>" alt="<?= e($titleText) ?>" class="item-card__image">
          <?php else: ?>
            <div class="item-card__no-image">No Image</div>
          <?php endif; ?>
        </a>
        <h3 class="item-card__title"><a href="<?= e($itemUrl) ?>"><?= e($titleText) ?></a></h3>
        <div class="item-card__buttons">
          <a href="<?= e($sampleMovieUrl) ?>" class="item-card__btn" target="_blank" rel="noopener noreferrer">サンプル動画</a>
          <a href="<?= e($sampleImagesUrl) ?>" class="item-card__btn" target="_blank" rel="noopener noreferrer">サンプル画像</a>
          <a href="<?= e($itemUrl) ?>" class="item-card__btn">詳細ページ</a>
        </div>
      </div>
    <?php endforeach; ?>
  </section>
