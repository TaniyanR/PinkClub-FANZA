...
$listingWhereParts = [];
if ($sourceWhere !== '') {
    $listingWhereParts[] = $sourceWhere;
}
$listingWhereParts[] = '(TRIM(COALESCE(items.title, "")) <> "" AND TRIM(COALESCE(items.title, "")) <> "タイトル未設定" AND (TRIM(COALESCE(items.image_small, "")) <> "" OR TRIM(COALESCE(items.image_large, "")) <> "" OR TRIM(COALESCE(items.image_list, "")) <> ""))';
$sourceWhereSql = ' WHERE ' . implode(' AND ', $listingWhereParts);
...
try {
    $stmt = db()->prepare('SELECT * FROM items' . $sourceWhereSql . ' ORDER BY release_date DESC, id DESC LIMIT :l OFFSET :o');
