...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!admin_post_csrf_valid()) {
        ...
        if ($action === 'save_sort_mode') {
            ...
            site_setting_set('links.sort_mode', $mode);
            admin_flash_set('ok', '表示順設定を保存しました。');
        } else {
            $id = (int)($_POST['id'] ?? 0);
            $status = (string)($_POST['status'] ?? 'pending');
            $displayOrder = (int)($_POST['display_order'] ?? 0);
            if ($id > 0 && in_array($status, ['pending', 'approved', 'rejected', 'hold'], true)) {
                db()->prepare('UPDATE mutual_links SET status=:status, display_order=:display_order, approved_at=CASE WHEN :status="approved" AND approved_at IS NULL THEN NOW() ELSE approved_at END, updated_at=NOW() WHERE id=:id')
                    ->execute([':status' => $status, ':display_order' => $displayOrder, ':id' => $id]);
...
<div class="admin-card"><table class="admin-table"><thead><tr><th>ID</th><th>サイト名</th><th>URL</th><th>表示順</th><th>ステータス</th><th>操作</th></tr></thead><tbody>
<?php foreach ($rows as $r) : ?>
<tr><td><?php echo e((string)$r['id']); ?></td><td><?php echo e((string)$r['site_name']); ?></td><td><a href="<?php echo e((string)$r['site_url']); ?>" target="_blank" rel="noopener"><?php echo e((string)$r['site_url']); ?></a></td><td>
<form method="post"><input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>"><input type="hidden" name="id" value="<?php echo e((string)$r['id']); ?>"><input type="number" name="display_order" value="<?php echo e((string)($r['display_order'] ?? 0)); ?>" style="width:90px"></td><td><select name="status"><?php foreach (['pending' => '保留', 'approved' => '承認', 'hold' => '保留', 'rejected' => '却下'] as $v => $lbl) : ?><option value="<?php echo e($v); ?>" <?php echo ((string)$r['status'] === $v) ? 'selected' : ''; ?>><?php echo e($lbl); ?></option><?php endforeach; ?></select></td><td><button type="submit">更新</button></form></td></tr>
<?php endforeach; ?>
<?php if ($rows === []) : ?><tr><td colspan="6">申請はまだありません。</td></tr><?php endif; ?>
</tbody></table></div>
<?php
