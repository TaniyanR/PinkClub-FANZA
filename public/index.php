...
function render_item_card(array $item, int $width = 180, ?array $taxonomy = null): void
{
    ...
    <article class="card rail-card rail-card--<?= (int)$width ?>">
      ...
      <div class="sample-buttons">
        <button type="button" class="<?= e($movieClass) ?>" <?= $sample['movie_url'] === '' ? 'disabled' : '' ?> onclick="<?= $sample['movie_url'] !== '' ? "window.open('" . e((string)$sample['movie_url']) . "','sample_movie_window','noopener,noreferrer,width=820,height=520');" : 'return false;' ?>">サンプル動画</button>
