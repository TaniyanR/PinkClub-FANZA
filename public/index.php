...
function is_likely_movie_url(string $url): bool
{
    ...
    foreach (['.mp4', '.m3u8', '.ism', '.ismv', '.flv', '.mov', '.webm'] as $ext) {
        if (str_contains($target, $ext)) {
            return true;
        }
    }

    return str_contains($target, '/litevideo/') || str_contains($target, '/sample/') || str_contains($target, '/vrsample/');
}
