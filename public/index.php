...
function render_item_card(array $item, int $width = 180, ?array $taxonomy = null): void
{
    ...
    ?>
    <article class="card rail-card rail-card--<?= (int)$width ?>" style="width:<?= (int)$width ?>px;min-width:<?= (int)$width ?>px;max-width:<?= (int)$width ?>px;box-sizing:border-box;">
      <?php if (!empty($item['image_small'])): ?>
        <img class="thumb" src="<?= e((string)$item['image_small']) ?>" alt="<?= e($title) ?>" style="width:100%;max-width:100%;">
      <?php else: ?>
        <div class="rail-card__noimage" style="width:100%;height:<?= (int)$width ?>px;">画像なし</div>
      <?php endif; ?>
      <a class="rail-card__title" href="<?= e($itemUrl) ?>" style="display:block;line-height:1.4;min-height:calc(1.4em * 2.5);max-height:calc(1.4em * 2.5);overflow:hidden;"><?= e($title) ?></a>
...
<?php else: ?>
  <section class="rail-section">
    ...
    <div class="rail-row rail-row--180 rail-row--no-scroll"><?php foreach ($latestBottom as $item) { render_item_card($item, 180); } ?></div>
  </section>
  ...
  <section class="rail-section">
    ...
    <div class="rail-row rail-row--180 rail-row--no-scroll"><?php foreach ($pickupBottom as $item) { render_item_card($item, 180); } ?></div>
  </section>
