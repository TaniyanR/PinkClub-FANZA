...
function pcf_sync_series_items_from_api(array $series, int $page, int $per): array
{
    ...
    if ($dmmId === '' || str_starts_with($dmmId, 'name:')) {
        return ['attempted' => false, 'total_count' => 0, 'content_ids' => []];
    }
    ...
    try {
        ...
        return ['attempted' => true, 'total_count' => max(0, (int)($result['total_count'] ?? 0)), 'content_ids' => $contentIds];
    } catch (Throwable $e) {
        error_log('series_detail.php ItemList series sync failed: ' . $e->getMessage());
        return ['attempted' => true, 'total_count' => 0, 'content_ids' => []];
    }
}
...
try {
    ...
    if ($series !== null) {
        ...
        $apiAttempted = (bool)($apiResult['attempted'] ?? false);
        $apiTotal = (int)($apiResult['total_count'] ?? 0);
        $apiContentIds = is_array($apiResult['content_ids'] ?? null) ? $apiResult['content_ids'] : [];
        $total = $apiAttempted ? $apiTotal : (function_exists('count_items_by_series') ? count_items_by_series((int)$series['id']) : 0);
