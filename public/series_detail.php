...
function pcf_series_raw_list(mixed $value): array
{
    if (!is_array($value)) {
        return [];
    }
...
function pcf_item_matches_series_raw(array $item, string $seriesName, string $seriesDmmId): bool
{
    ...
    $seriesDmmId = trim($seriesDmmId);
    foreach (pcf_series_raw_list($raw['iteminfo']['series'] ?? ($raw['series'] ?? [])) as $seriesRow) {
        if (!is_array($seriesRow)) {
            continue;
        }
        $rowId = trim((string)($seriesRow['id'] ?? ''));
        $rowName = trim((string)($seriesRow['name'] ?? ''));
        if ($seriesDmmId !== '' && !pcf_series_use_name_lookup($seriesDmmId)) {
            if ($rowId === $seriesDmmId) {
                return true;
            }
            continue;
        }
        if ($seriesName !== '' && $rowName === $seriesName) {
            return true;
        }
    }
...
function pcf_series_raw_like_value(string $seriesName, string $seriesDmmId): string
{
    $seriesDmmId = trim($seriesDmmId);
    if ($seriesDmmId !== '' && !pcf_series_use_name_lookup($seriesDmmId)) {
        return $seriesDmmId;
...
function pcf_count_series_items_from_raw(string $seriesName, string $seriesDmmId): int
{
    $likeValue = pcf_series_raw_like_value($seriesName, $seriesDmmId);
    if ($likeValue === '') {
        return 0;
    }
    ...
    do {
        try {
            $stmt = db()->prepare(
                'SELECT id, raw_json
                 FROM items
                 WHERE raw_json LIKE :needle' . pcf_series_item_source_sql('items') . '
                 ORDER BY items.release_date DESC, items.id DESC
                 LIMIT :limit OFFSET :offset'
            );
            $stmt->bindValue(':needle', '%' . addcslashes($likeValue, '\\%_') . '%', PDO::PARAM_STR);
        ...
        foreach ($rows as $row) {
            if (is_array($row) && pcf_item_matches_series_raw($row, $seriesName, $seriesDmmId)) {
                $count++;
            }
        }
        $offset += $chunk;
...
function pcf_fetch_series_items_from_raw(string $seriesName, string $seriesDmmId, int $limit, int $offset): array
{
    $likeValue = pcf_series_raw_like_value($seriesName, $seriesDmmId);
    if ($likeValue === '') {
        return [];
    ...
    do {
        try {
            ...
            $stmt->bindValue(':needle', '%' . addcslashes($likeValue, '\\%_') . '%', PDO::PARAM_STR);
            $stmt->bindValue(':limit', $chunk, PDO::PARAM_INT);
            $stmt->bindValue(':offset', $cursor, PDO::PARAM_INT);
            $stmt->execute();
            $rows = $stmt->fetchAll() ?: [];
        } catch (Throwable) {
            return [];
...
function pcf_series_count_items(string $seriesName, string $seriesDmmId): int
{
    ...
    $rawCount = pcf_count_series_items_from_raw($seriesName, $seriesDmmId);
    if ($rawCount > 0) {
        return $rawCount;
    }
...
function pcf_fetch_series_items(string $seriesName, string $seriesDmmId, int $limit, int $offset): array
{
    ...
    $rawItems = pcf_fetch_series_items_from_raw($seriesName, $seriesDmmId, $limit, $offset);
    if ($rawItems !== []) {
        return $rawItems;
    }
