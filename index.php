...
try {
    ...
    if ($itemCount > 0) {
        ...
        $popularRows = fetch_items_with_order_fallback($pdo, [
            'CASE WHEN view_count > 0 THEN 0 ELSE 1 END ASC, view_count DESC, release_date DESC, id DESC',
            'CASE WHEN view_count > 0 THEN 0 ELSE 1 END ASC, view_count DESC, date_published DESC, id DESC',
            'CASE WHEN view_count > 0 THEN 0 ELSE 1 END ASC, view_count DESC, id DESC',
            'id DESC',
        ], 40);
        $popularRows = array_values(array_filter($popularRows, static function ($item): bool {
            return is_array($item) && (int)($item['view_count'] ?? 0) > 0;
        }));
